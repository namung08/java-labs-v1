...
public class StreamLab {
    ...
    public static void main(String[] args) {
        ...
        Stream<Integer> stream = IntStream.rangeClosed(1, 20).boxed();
        stream.forEach(System.out::println);

        // TODO: 문자열 배열에서 스트림을 생성하여 출력하세요.
        String[] subjects = {"국어", "영어", "수학", "과학", "사회"};
        // 힌트: Arrays.stream(subjects)...
        List<String> subjectsList = Arrays.stream(subjects).toList();

        // TODO: 리스트에서 스트림을 생성하여 출력하세요.
        List<String> cities = Arrays.asList("서울", "부산", "인천", "대구", "대전", "광주", "울산", "세종", "테스트 도시", "test");
        // 힌트: cities.stream()...
        ...
        numbers.stream().filter(n -> n % 2 == 0).map(n -> n * n).mapToInt(Integer::intValue).average().stream().forEach(System.out::println);

        // TODO: 문자열 리스트에서 길이가 3 이상인 문자열만 대문자로 변환하세요.
        // 힌트: cities.stream().filter(...).map(...)
        cities.stream().filter(s -> s.length() >= 3).map(String::toUpperCase).forEach(System.out::println);
        ...
        Map<String, Long> departmentCount = students.stream().collect(Collectors.groupingBy(Student::getDepartment, Collectors.counting()));
        departmentCount.forEach((String department, Long count) -> {
        ...
        Map<String, Double> departmentAvg = students.stream().collect(Collectors.groupingBy(Student::getDepartment, Collectors.averagingInt(Student::getScore)));
        departmentAvg.forEach((department, avg) -> {
            System.out.println(department + " avg : " + avg);
        });
        // TODO: 학년별로 그룹화한 후 최고 점수를 받은 학생을 찾으세요.
        // 힌트: students.stream().collect(Collectors.groupingBy(..., Collectors.maxBy(...)))
        Map<Integer, Optional<Student>> maxStudent = students.stream().collect(Collectors.groupingBy(Student::getGrade, Collectors.maxBy(Comparator.comparing(Student::getScore))));
        maxStudent.forEach((grade, student) -> {
            System.out.println(grade + " : " + student.map(Student::toString).orElse("없음"));
        });
        ...
        Map<String, Long> wordCount = Arrays.stream(text.split("\\s+|\\.|")).filter(s -> !s.isEmpty()).collect(Collectors.groupingBy(word -> word, Collectors.counting()));
